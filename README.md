...
- 기술 태그 
- GitHub / Blog 버튼 2개
- 온라인 상태 뱃지 (오프라인으로 변경하냐 마냐의 여부는 추후 추가로 구현 예정)
...
- `@keyframes popIn` — 아바타가 카드보다 아주 살짝 늦게 등장하는 딜레이 애니메이션 적용
- `@keyframes pulse` — 상태 뱃지의 초록 점이 천천히 깜빡이는 효과 적용
- `dark:` 접두사를 활용한 다크모드 스타일 적용 및 토글 버튼 구현
- 카드 2개를 `sm:grid-cols-2`로 데스크탑에서 2열 grid 배치
- `card--red` Modifier로 색깔을 바꿔 빨간색 카드 변형 추가 
+  모바일(560px 미만)에서 카드가 가로 레이아웃으로 전환되는 반응형 구성(AI의 도움으로 구현..)
+  
## 5. 비교 및 회고

### 1) BEM 방식으로 다시 설계

내가 만든 카드 UI를 BEM 방식으로 설계하면 클래스명을 아래와 같이 구성가능하다.
...
| HTML 클래스 길이 | 길다 | 짧다. 명확하다. |
| CSS 파일 | 없음 | 별도로 작성이 필요하다 |
| 카드 색상 변형 | 색상 클래스를 전부 다시 재작성해야하는 번거로움이 있다. | `card--red` 한 줄만 추가하면 쉽게 변경이 가능하다. |
| 다크모드 | `dark:` 클래스 인라인 추가로 번거롭다. | `body.is-dark .card` 로 일괄 처리가 가능해 간단하다. |
| 가독성 | 파악하기 어렵다. | 클래스명만 봐도 구조 파악 가능해 가독성이 높다. |
| 개발 속도 | 빠르다 | 느리다. |
| 유지보수 | 수정 시 HTML을 직접 일일이 찾아야 해서 유지보수가 어렵다. | CSS 파일에서 일괄 수정이 가능해 유지보수가 용이하다. |
